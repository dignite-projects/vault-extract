...
| `tools/call` → `search_documents` | Structured search **within a required `documentTypeCode`**: metadata (`lifecycleStatus`) + zero or more `ExtractedFields` field-value filters, all combined with **AND** (each is an equality, or a numeric/date `min`/`max` range). No keyword/full-text search. Returns an object with `items` (up to 50 thin rows, each carrying the `vault-extract://documents/{id}` uri to read the full document) plus `totalCount` and `truncated`; when `truncated` is true, more documents matched than were returned, so the client should narrow the query rather than treat `items` as complete |
...
2. The tool returns an object with `items` (thin rows, each with a `vault-extract://documents/{id}` uri) plus `totalCount` and `truncated` — if `truncated` is true, the client narrows the query rather than assuming `items` is the complete match set.
