...
Every request to `/mcp` requires a valid Bearer token (`RequireAuthorization` on the endpoint). In addition, each tool/resource call performs an explicit server-side permission assertion: the caller must be granted **`VaultExtract.Documents`** (`VaultExtractPermissions.Documents.Default`). A token without that permission gets an authorization error even though the endpoint accepted the connection (fail-closed, defense in depth).

> **`VaultExtract.Documents` is entry, not read.** Since the per-document-type permissions shipped, it admits the caller to the MCP surface but does not by itself return any document. A client also needs **`VaultExtract.Documents.ReadAll`** to see every type of its layer, or a per-type `Read` grant for each type it should see — `search_documents`, `get_document` and the document resources all inherit the same read scope as the operator list, and documents that carry no type at all (unclassified, failed classification, containers) are reachable only with `ReadAll`. An existing MCP OAuth client that held only `VaultExtract.Documents` returns an empty search until it is granted `ReadAll`. See [per-document-type permissions](../configuration/document-type-permissions.md).
