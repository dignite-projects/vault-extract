...
- **An unclassified document is invisible to a caller without `Documents.ReadAll`.** Until the classification stage assigns a type, the document belongs to no type, and no per-type grant can name it — so it is excluded from the document list, the export and the MCP search of every caller narrowed to per-type `Read` grants, and `GetAsync` on it is denied to them. The same applies to a document whose classification failed and to a container. Reviewing the unresolved-classification queue therefore requires `ReadAll`. See [per-document-type permissions](../configuration/document-type-permissions.md).
