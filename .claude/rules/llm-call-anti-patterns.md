---
description: "Document AI internal LLM call-site anti-patterns: fail-closed security gate / PromptBoundary / compile-time-constant descriptions / MCP schema collection-parameter pitfall"
paths:
  - "**/*Workflow*.cs"
  - "**/Pipelines/**/*.cs"
  - "**/*Extraction*.cs"
...
This file is referenced by the `maf-workflow-reviewer` agent during PR review to quickly locate typical mistakes at Document AI's internal **LLM call sites** (anti-patterns A / B are security issues, anti-pattern C is an MCP schema correctness issue). The rules apply to all LLM entry points:

- Currently landed: `DocumentClassificationWorkflow` / `FieldExtractionWorkflow` + `FieldExtractionEventHandler` (field architecture v2 unifying Host + tenant (mechanism B)) / `DocumentTextExtractionBackgroundJob.TryGenerateTitleAsync`
...
- RAG retrieval injects chunks from other, unrelated documents into the prompt, causing structured fields like "contract amount" and "party A name" to be extracted **from the wrong document** and written into a downstream business aggregate root / the Document AI type-bound field table
- Document AI is a **channel layer**; the input to field extraction should be **only the current document's Markdown** — attaching `AIContextProviders` corrupts the channel philosophy into RAG, violating CLAUDE.md's "OUT of scope"
...
```csharp
[McpTool("search-documents")]
[Description("Search Document AI documents by structured criteria.")]   // ← compile-time constant
