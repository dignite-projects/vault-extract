---
description: "Document AI field architecture details: system common field table, type-bound field (mechanism B) implementation, field-extension judgment, document-type classification execution"
paths:
  - "**/Document.cs"
  - "**/Documents/**/*.cs"
  - "**/*Field*.cs"
...
# Field Architecture Details (Document AI)
...
## System common fields (auto-produced by the Document AI pipeline, top-level typed columns)

Produced automatically by the Document AI pipeline + built-in LLM extraction, applicable to all documents, **requiring no schema configuration**. Stored as top-level typed columns on `Document` (strongly-typed LINQ + first-class indexes):
...
- On upload, Document AI automatically runs the LLM classification prompt → categorizes within the layer the Document belongs to (exact match by `Document.TenantId`; the background path switches layer via `ICurrentTenant.Change` then goes through the generic `GetListAsync`)
...
**Essence of mechanism (B)**: Document AI provides a generic "extract-by-schema" engine — Host or tenant configures the schema, and the engine extracts per the owning layer. Document AI Core **presets no business field definition** (contract amount / invoice number / tax amount, etc. are not hardcoded).
