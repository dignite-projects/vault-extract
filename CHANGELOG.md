...
- **BREAKING — `VaultExtract.Documents` no longer means "read every document"; a new child `VaultExtract.Documents.ReadAll` does** ([#632](https://github.com/dignite-projects/vault-extract/issues/632)). `VaultExtract.Documents` keeps its place as the permission-group parent and the SPA route gate, and now means only "may enter the documents area and work inside the caller's own type scope". The split is what makes a per-type `Read` grant expressible at all: ABP's `PermissionDefinition.Parent` contract is "a child can only be granted if the parent is", and `Documents` is the parent of `Upload`, `Delete`, `ConfirmClassification`, `Export`, `Restore`, `PermanentDelete`, `Pipelines.*` and `Reprocessing.*` — so every principal that can reach the documents area or upload anything already held it, and a per-type `Read` hung off it alone would never have narrowed anyone. The overview statistics endpoint moves to `ReadAll` too: a whole-layer aggregate has no per-type meaning.
