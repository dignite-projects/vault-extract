...
import { rightsOf } from '../../shared/document-access';
...
export class DocumentRecycleBinComponent implements OnInit {
  ...
  // #635 decision 5: the restore right rides down with the row. Reading `rights` per row replaces the
  // client-side rule table this page used to run against the visible types — which is also why this page no
  // longer fetches them: nothing else on it was type-derived.
  ...
  readonly hasRecycleActions = computed(
    () => this.canPermanentDelete || this.documents().items.some(d => rightsOf(d).canRestore),
  );
  ...
  // #635 decision 6: the query is always hooked. Admission to the recycle bin is now entry alone and its rows
  // are the caller's read scope — own documents included — so the server always has an honest answer, and an
  // empty bin is empty by construction. The three-way conditional start this page used to do (fetch the
  // types, decide whether the caller may restore anything, hook the query only then) had two states in which
  // no query existed at all, which is what the hooked-yet latch and the dual-meaning refresh() coped with.
  ngOnInit(): void {
    this.hookListQuery();
  }

  /** The header's Refresh button. One meaning: re-run the list query. */
  refresh(): void {
